...
const FEATURED_TEMPLATES = [
    {
		...
		description: 'E-Commerce Backend. This is the default starter template (recommended)',
	},
    {
		name: 'video-library',
...
const init = async () => {
            ...
            choices: FEATURED_TEMPLATES.map((template) => {
                if (typeof template === 'string') {
                    ...
                    return {
                        value: template.name,
                        title:
                            chalk.bold(padEnd(template.name, descriptionColumn)),
                        path: template.path,
                    };
                }
            }),
    ...
    try {
		const emitter = degit(`https://github.com/neogcamp/mockBee/apps/${template}`);
	...
	Log.info(`✨ Your mock backend template has been created at ${chalk.blue(folderName)}.\n`);
	...
	Log.info(
		...
		chalk.underline('https://neog.camp')
	);
