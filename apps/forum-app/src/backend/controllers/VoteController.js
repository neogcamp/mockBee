 ...
 * */
 ...
 export const getAnswerVotesHandler = function (schema, request) {
    ...
    try {
        const question = this.db.questions.findBy({ _id: questionId });
        const {votes} = question.answers.find(answer => answer._id === answerId) 
   ...
   export const voteQuestionHandler = function (schema, request) {
    ...
    try {
        ...
        switch(vote.reaction){
            case "upvote":
                question.votes.upvotedBy = question.votes.upvotedBy.filter(username => username !== user.username)
                question.votes.downvotedBy = question.votes.downvotedBy.filter(username => username !== user.username)
            ...
            case "downvote":
                ...
                break;
            default:
                return new Response(400, {}, { errors:["Invalid Reaction. Please Try with a valid Reaction again."] });
 ...
 * */
   ...
   export const voteAnswerHandler = function (schema, request) {
    ...
    try {
        ...
        switch(vote.reaction){
            case "upvote":
                answer.votes.upvotedBy = answer.votes.upvotedBy.filter(username => username !== user.username)
                answer.votes.downvotedBy = answer.votes.downvotedBy.filter(username => username !== user.username)
