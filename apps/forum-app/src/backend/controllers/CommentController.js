 ...
 * */
 ...
 export const getAnswerCommentsHandler = function (schema, request) {
    ...
    try {
        const question = this.db.questions.findBy({ _id: questionId });
        const {comments} = question.answers.find(answer => answer._id === answerId) 
  ...
  export const editQuestionCommentHandler = function (schema, request) {
    ...
    try {
      ...
      if(question.comments[commentIndex].username !== user.username){
        return new Response(400, {},  { errors: ["Cannot edit a comment doesn't belong to the User."]});    
      }
      question.comments[commentIndex] = {...question.comments[commentIndex], ...commentData, updatedAt: new Date().toDateString()}
      this.db.questions.update({_id: questionId}, question)
      return new Response(201, {}, { questions: this.db.questions }); 
    } catch (error) {
      return new Response(
 ...
 * */
  ...
  export const deleteQuestionCommentHandler = function (schema, request) {
    ...
    try {
      ...
      const question = this.db.questions.findBy({_id: questionId});
      const commentIndex = question.comments.findIndex(comment => comment._id === commentId);
      if(question.comments[commentIndex].username !== user.username && question.username !== user.username){
        return new Response(400, {},  { errors: ["Cannot delete a comment doesn't belong to the User."]});    
      }
      question.comments = question.comments.filter(comment => comment._id !==  commentId)
      this.db.questions.update({_id: questionId}, question)
      return new Response(201, {}, { questions: this.db.questions }); 
    } catch (error) {
      return new Response(
 ...
 * */
    ...
    export const editAnswerCommentHandler = function (schema, request) {
        ...
        try {
          ...
          const question = this.db.questions.findBy({_id: questionId})
          const answer = question.answers.find(answer => answer._id === answerId);
          const commentIndex = answer.comments.findIndex(comment => comment._id === commentId);
          if(answer.comments[commentIndex].username !== user.username && answer.username !== user.username){
            return new Response(400, {},  { errors: ["Cannot edit a comment doesn't belong to the User."]});    
          }
          answer.comments[commentIndex] = {...answer.comments[commentIndex], ...commentData, updatedAt: new Date().toDateString()}
          this.db.questions.update({_id: questionId}, question)
          return new Response(201, {}, { questions: this.db.questions }); 
        } catch (error) {
          return new Response(
 ...
 * */
  ...
  export const deleteAnswerCommentHandler = function (schema, request) {
    ...
    try {
      ...
      answer.comments = answer.comments.filter(comment => comment._id !==  commentId)
      this.db.questions.update({_id: questionId}, question)
      return new Response(201, {}, { questions: this.db.questions }); 
    } catch (error) {
      return new Response(
