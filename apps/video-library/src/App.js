...
export default function App () {
  ...
  return (
    <div>
          ...
          {videos.map((video, index) => {
            return (
              <tr key={index}>
                ...
                <td>
                  <td>
                    <button onClick={() => addToLikedVideos(video)}>
                      {' '}
                      🚀{' '}
                    </button>
                    <button onClick={() => removeFromLikedVideos(video)}>
                      {' '}
                      🕯{' '}
                    </button>
      ...
      <div>
        <h1>
          History Videos <button onClick={() => clearHistory()}>Clear</button>
        </h1>
        <ul>
          {history.map((video) => (
            <div>
      ...
      <div>
        <h1>Playlists </h1>
        <div>
          <button onClick={() => createPlaylist({ title: 'Watch Later' })}>
            Create Playlist
          </button>
        </div>
        <ul>
            ...
            <div>
              ...
              <li>
                ...
                {playlist.videos.map((video) => (
                  <div>
                    {video.title}
                    <button onClick={() => removeVideoFromPlaylist(video)}>
                      Remove From Videos
                    </button>
                  </div>
                ))}
